        ...
        <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          <form class="space-y-6" @submit.prevent="onSubmit">
            ...
            <button
              ...
              class="group relative w-full flex justify-center items-center py-2 px-4 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200"
              :disabled="loading"
            >
...
const loginWithGoogle = async () => {
  ...
  try {
    ...
    if (result.success) {
      toast.add({
        title: "ログイン成功",
        description: "Googleでログインに成功しました",
        color: "success",
      });
    ...
    let message = "Googleログイン処理中にエラーが発生しました";
    if (error instanceof FetchError && error.data) {
      const errorData = error.data as { message?: string };
      if (errorData.message) {
        message = errorData.message;
